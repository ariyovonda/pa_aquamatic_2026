...
const SENSOR_META = {
  temperature: { label: 'Temperature', fullLabel: 'Water Temperature', icon: 'thermometer', color: '#ef4444', bg: '#fee2e2', desc: 'Pond water temperature' },
  ph: { label: 'pH Level', fullLabel: 'pH Level', icon: 'ph', color: '#22c55e', bg: '#dcfce7', desc: 'Water acidity level' },
  tds: { label: 'TDS Level', fullLabel: 'TDS Level', icon: 'tds', color: '#a855f7', bg: '#f3e8ff', desc: 'Nutrient concentration' },
  do: { label: 'Dissolved O2', fullLabel: 'Dissolved Oxygen', icon: 'oxygen', color: '#3b82f6', bg: '#dbeafe', desc: 'Oxygen level in water' },
  turbidity: { label: 'Turbidity', fullLabel: 'Turbidity', icon: 'turbidity', color: '#f97316', bg: '#ffedd5', desc: 'Water clarity' },
};
...
export default function DashboardPage() {
  const { sensorData, sensorEnabled, toggleSensor, firebaseConnected } = useApp();
  ...
  useEffect(() => {
    ...
    const from = getRangeCutoff(chartRange);
    const to = new Date();
    getAllSensorsHistory(from, to, 500)
      .then(data => {
  ...
  const allNormal = Object.values(sensorData).every(s => s.status === 'normal');
    ...
    <div className="dashboard-page">
      ...
      <div className="db-status-bar">
        <span className={`db-badge ${firebaseConnected ? 'live' : 'sim'}`}>
          ...
          {firebaseConnected ? 'Firebase Realtime DB – Live' : 'Simulation Mode (Firebase not configured)'}
        </span>
      </div>
      ...
      <div className="sensor-grid">
        {Object.entries(sensorData).map(([key, sensor]) => (
          <SensorCard
            ...
            enabled={sensorEnabled[key]}
            onToggle={() => toggleSensor(key)}
          />
        ))}
      </div>
        ...
        <div className="sensor-tabs">
          {Object.keys(SENSOR_META).map(key => (
            <button
              ...
              style={activeTab === key ? { color: SENSOR_COLORS[key], borderColor: SENSOR_COLORS[key] } : {}}
              onClick={() => setActiveTab(key)}
            >{SENSOR_META[key].label}</button>
          ))}
          <button
            className={`sensor-tab ${activeTab === 'all' ? 'active' : ''}`}
            style={activeTab === 'all' ? { color: '#0d9488', borderColor: '#0d9488' } : {}}
